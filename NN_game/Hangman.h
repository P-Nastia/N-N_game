...
public:
    ...
    void showHiddenWord(RenderWindow& hangmanWindow) {//виводиться все слово
        for (int i = 0; i < wordLength; i++) {
            hangmanWindow.draw(separatedLettersOfWord[i]);
        }
    }
    void showAlhpabet(RenderWindow& hangmanWindow) {//виводиться алфавіт, по якому власне гравець рухом клавіш проходиться та обирає букву
        for (int i = 0; i < 26; i++) {
